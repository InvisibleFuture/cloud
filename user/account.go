...
func (a *Account) Register(account, password string) bool {
	if _, err := DB_ACCOUNT.Get([]byte(account), nil); err == nil {
		return false
	}
	if account == "" || password == "" || len(account) > 32 || len(password) > 32 {
		return false
	}
	num := <-CH_USERID
...
func Identity(uid, token string) bool {
	if t, ok := CA_TOKEN.Load(uid); !ok || t != token {
		return false
	}
	return true
}
